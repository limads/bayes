...
/// most posteriors, as long as they don't have a very high dimensionality. Basically,
/// the algorithm calculates a sequence of posteiror log-odds values (which do not depend on
/// the normalization factor of Bayes' theorem), using this log-odds to decide on whether to
/// re-sample a random walk at the current parameter value or to move the random walk to
    ...
    O : ?Sized
{
    ...
    let curr_lp : f64 = lik.joint_log_prob().ok_or(format!("Error evaluating log-probability"))?;
...
fn native_metropolis<O : ?Sized>(
    ...
    let mut prev_lp = lik.joint_log_prob().ok_or(format!("Error evaluating log-probability"))?;
