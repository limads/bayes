...
/// the generative model L, which is a top-level likelihood node L plus any prior factors. If there are
/// multiple algorithms to estiamate any (Posterior, Likelihood) pair, the method is disambiguated by
/// passing Some(algorithm) to the fit method, or else a standard algorithm is used. If there is a single
/// method to fit this pair, and the algorithm takes no Parameters, Algorithm can be defined as () and the
/// user can pass either Some(()) or None. If there is a single method to fit this pair and the algorithm
/// takes any parameters, the paraemters can be passed as state into the Some(alg) object.
...
/*
Define trait Fit with standard implementation:
...
pub trait Fit {
    ...
    type Estimator : Default;

    fn fit(&mut self, estimator : Estimator) -> Result<Posterior, Error>;

}

/// Use fit if you will use your estimator only once, since fit will
/// take ownership of the estimator. Use estimator.estimate(&mut lik)
/// if you will make the call several times.
impl<O, P, S, E> Fit for L
where
    L : Likelihood<O>,
    P : Posterior,
    E : Error,
    S : Estimator<P, E> + Default
{

    fn fit(&mut self, est : S) -> Result<P, E> {
        est.estimate(self)
    }
}

All graphs are defined to have at least one "standard" estimator, to make the user be able to:

let a = b.fit(Default::default());
