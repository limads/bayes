...
pub struct NormalDecl {
    pub mean : f64,
    pub var : f64,
    joint : Option<Vec<NormalDecl>>,
    // obs : Option<Vec<f64>>
}

impl From<NormalDecl> for Normal {
    fn from(decl : NormalDecl) -> Self {
        Normal::new(1, Some(decl.mean), Some(decl.var))
    }
}

impl From<Normal> for NormalDecl {
    fn from(norm : Normal) -> Self {
        NormalDecl { mean : norm.mean()[0], var : norm.var, joint : None }
    }
}

impl TryFrom<serde_json::Value> for Normal {

    type Error = ();

    fn try_from(v : Value) -> Result<Self, Self::Error> {
        let mean : f64 = v.get("mean").ok_or(())?.as_f64().ok_or(())?;
        let var : f64 = v.get("var").ok_or(())?.as_f64().ok_or(())?;
