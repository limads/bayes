...
pub fn define_step(domain : &[f64], n : usize) -> f64 {
    let dom_start = domain[0];
    let dom_end = domain[domain.len()-1];
    (dom_end - dom_start).abs() / n as f64
}

pub fn interpolate(x : &[f64], y : &[f64], n : usize) -> Result<Vec<f64>, &'static str> {
    if x.len() != y.len() {
    ...
    unsafe {
        ...
        let spline = spline::gsl_spline_alloc(spline::gsl_interp_cspline, x.len());
        spline::gsl_spline_init (spline, &x[0] as *const f64, &y[0] as *const f64, x.len());
        ...
        spline::gsl_interp_accel_free (acc);
        Ok(interp)
    }
}

pub fn interpolate_auto(y : &[f64], n : usize) -> Result<Vec<f64>, &'static str> {
    let mut x = Vec::new();
    for i in 0..y.len() {
        x.push(i as f64)
    };
...
pub fn define_homogeneous_domain(from : f64, to : f64, n : usize) -> Vec<f64> {
