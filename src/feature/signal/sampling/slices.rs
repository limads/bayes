...
#[inline(always)]
pub fn convert_slice<M, N>(src : &[M], dst : &mut [N])
where
    N : Scalar + From<M>,
    M : Scalar + Copy
{
...
pub fn subsample_convert_window<'a, M, N>(
    src : &'a [M],
    offset : (usize, usize),
    src_dims : (usize, usize), 
    dst_dims : (usize, usize),
    step : usize,
    mut dst_rows : impl Iterator<Item=&'a mut [N]>
)
where
    M : Scalar + Copy,
    N : Scalar + From<M>
{
    let iter_rows = src.chunks(src_dims.0)
        .skip(offset.0)
        .step_by(step)
        .take(dst_dims.0);
    for (src_row, dst_row) in iter_rows.zip(dst_rows) {
        for (j, el) in src_row.iter().skip(offset.1).step_by(step).take(dst_dims.1).enumerate() {
            dst_row[j] = N::from(*el);
        }
    }
}
